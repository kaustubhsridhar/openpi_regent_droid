...
def main(args: Args):
    ...
    first_one = og_open_loop_horizon
    second_one = 5
    third_one = 10
    print("Entered main!")
        ...
        for t_step in bar:
            try:
                ...
                if actions_from_chunk_completed == 0 or actions_from_chunk_completed >= args.open_loop_horizon:
                    ...
                    if args.open_loop_horizon == first_one and switch_time is None and pred_action_chunk[-1, -1] > 0.9:
                        print(f'switch time is {t_step}')
                        switch_time = t_step

                    if args.open_loop_horizon == first_one and switch_time is not None and second_switch_time is None and t_step - switch_time > 10:
                        print(f"Switching to open loop horizon of {second_one} for grasping at second_switch_time of {t_step}")
                        args.open_loop_horizon = second_one # for grasping
                        second_switch_time = t_step

                    if args.open_loop_horizon == second_one and second_switch_time is not None and third_switch_time is None and t_step - second_switch_time > 10:
                        print(f"Switching to open loop horizon of {third_one} for lifting at third_switch_time of {t_step}")
                        args.open_loop_horizon = third_one # for lifting
                        third_switch_time = t_step
