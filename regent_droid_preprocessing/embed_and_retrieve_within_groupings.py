...
def retrieval_preprocessing(chosen_id_to_ep_idxs_with_atleast_min_num_episodes, ds_name, num_episodes_to_retrieve_from, nb_cores_autofaiss, knn_k, embedding_type):
	myprint(f'[retrieval_preprocessing] starting retrieval preprocessing for {embedding_type}')
	
	# init setup
	ds_emb_fol = f"{ds_name}_broken_up_embeddings"
	indices_fol = f"{ds_emb_fol}_broken_up_indices/numepisodes{num_episodes_to_retrieve_from}_embtype{embedding_type}_knnk{knn_k}"
	os.makedirs(f"{ds_emb_fol}_broken_up_indices", exist_ok=True)
	...
	for chosen_id_count, (chosen_id, ep_idxs) in enumerate(chosen_id_to_ep_idxs_with_atleast_min_num_episodes.items()):
		...
		for ep_count, ep_idx in enumerate(ep_idxs_capped):
			...
			elif isinstance(embedding_type, list):
				ep_embeddings = np.concatenate([np.load(f"{ds_emb_fol}/episode_{ep_idx}_{item}.npy") for item in embedding_type], axis=1)
				all_embeddings.append(ep_embeddings)
			else:
				raise ValueError(f'embedding_type must be a string or a list of strings, got {embedding_type}')
		...
		for ep_count, ep_idx in enumerate(ep_idxs_capped):
			all_other_episodes_mask = [1 if ep_idx_other != ep_idx else 0 for (ep_idx_other, step_idx_other) in all_indices]
			all_other_episodes_mask = np.array(all_other_episodes_mask)
			num_retrieval = np.sum(all_other_episodes_mask)
			this_episode_mask = [1 if ep_idx_other == ep_idx else 0 for (ep_idx_other, step_idx_other) in all_indices]
			this_episode_mask = np.array(this_episode_mask)
			...
			assert all_other_episodes_embeddings.shape == (num_retrieval, embedding_dim) and all_other_episodes_indices.shape == (num_retrieval, 2)
			...
			topk_distances, topk_indices = knn_index.query(this_episode_embeddings, knn_k)
			...
			myprint(f'[retrieval_preprocessing] saved retrieval indices for episode {ep_idx} [episode count {ep_count}/{num_episodes_to_retrieve_from}]')
...
if __name__ == "__main__":
	...
	parser.add_argument("--knn_k", type=int, default=20, help="number of nearest neighbors to retrieve")
	...
	# 						embedding_type="embeddings__exterior_image_1_left")
	# retrieval_preprocessing(chosen_id_to_ep_idxs_with_atleast_min_num_episodes, ds_name, args.num_episodes_to_retrieve_from_in_each_grouping, args.nb_cores_autofaiss, args.knn_k,
	# 						embedding_type="embeddings__exterior_image_2_left")
	# retrieval_preprocessing(chosen_id_to_ep_idxs_with_atleast_min_num_episodes, ds_name, args.num_episodes_to_retrieve_from_in_each_grouping, args.nb_cores_autofaiss, args.knn_k,
	# 						embedding_type="embeddings__wrist_image_left")
	# retrieval_preprocessing(chosen_id_to_ep_idxs_with_atleast_min_num_episodes, ds_name, args.num_episodes_to_retrieve_from_in_each_grouping, args.nb_cores_autofaiss, args.knn_k,
	# 						embedding_type=["embeddings__exterior_image_1_left", "embeddings__wrist_image_left"])
	# retrieval_preprocessing(chosen_id_to_ep_idxs_with_atleast_min_num_episodes, ds_name, args.num_episodes_to_retrieve_from_in_each_grouping, args.nb_cores_autofaiss, args.knn_k,
	# 						embedding_type=["embeddings__exterior_image_1_left", "embeddings__exterior_image_2_left", "embeddings__wrist_image_left"])
