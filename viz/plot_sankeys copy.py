...
def plot_sankey(task_name, quantitative_results):
    ...
    if num_stages == 2:
        ...
        for i in range(num_methods):
            ...
            values.append(first_stage_success[i])
            values.append(first_stage_fails[i])
            links_to_methods.append(methods[i])
            links_to_methods.append(methods[i])
        for i in range(num_methods):
            ...
            values.append(second_stage_success[i])
            values.append(second_stage_fails[i])
            links_to_methods.append(methods[i])
            links_to_methods.append(methods[i])
    elif num_stages == 3:
        source_nodes = []
        ...
        for i in range(num_methods):
            ...
            values.append(third_stage_success[i])
            values.append(third_stage_fails[i])
            links_to_methods.append(methods[i])
            links_to_methods.append(methods[i]) 
    else:
    ...
    method_to_colors = {
            ...
            "Retrieve and play": palette_colors[4],
            ...
            "Diffusion Policy (from scratch)": palette_colors[5],
        }
    ...
    fig = go.Figure(data=[go.Sankey(
        node = dict(
            ...
            color = ["black"] + sum([["green", "red"] for s in stages], []),
            align='left',
            # x = node_x,
            # y = node_y,
        ),
